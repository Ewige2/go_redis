...
type readState struct {
	readingMultiLine  bool //  判断是否读取新回复的首行
	expectedArgsCount int  //  期望的参数个数
	msgType           byte
	args              [][]byte
	bulkLen           int64 //  二进制安全的BulkString
}
...
func parse0(reader io.Reader, ch chan<- *Payload) {
	...
	for {
		...
		// 解析一行数据
		if !state.readingMultiLine { //  是否读取多行数据
